const themes = [
  ...
  { label: "cyan", image: "./images/cyan.jpg" },
  { label: "pink", image: "./images/pink.jpg" },
  { label: "orange", image: "./images/orange.jpg" },
  { label: "purple", image: "./images/purple.jpg" },
  { label: "dark", image: "./images/dark.jpg" },
];
...
window.addEventListener("DOMContentLoaded", async () => {
  try {
    ...
    document.getElementById(
      "feelsLike"
    ).textContent = `Feels ${feelsLikeCelsius}°C`;
    ...
    if (humidity > 40) {
      document.getElementById(
        "slider"
      ).style.width = `calc(${humidity}% - 60px)`;
    }
...
setInterval(() => {
  ...
  document.getElementById(
    "hour"
  ).style.transform = `rotate(${hour_rotate_angle}deg)`;
  ...
  document.getElementById("date").innerText = `${dayName.substring(
    0,
    3
...
const changeBackgroundImageForTheme = (themeKey) => {
  const isEnabled =
    localStorage.getItem("enableBackgroundImageState") === "checked"
      ? true
      : false;
  if (isEnabled) {
    const themeObject = themes.find((theme) => theme.label === themeKey);
    document.body.style.backgroundImage = `url(${themeObject.image})`;
...
const applySelectedTheme = (colorValue) => {
  changeBackgroundImageForTheme(colorValue);
  if (colorValue != "blue") {
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const enableBackgroundImage = document.getElementById(
  ...
  loadCheckboxState("enableBackgroundImageState", enableBackgroundImage);
  ...
  enableBackgroundImage.addEventListener("change", function () {
    saveCheckboxState("enableBackgroundImageState", enableBackgroundImage);
    const selectedTheme = localStorage.getItem("selectedTheme");
    changeBackgroundImageForTheme(selectedTheme);
  });
});
